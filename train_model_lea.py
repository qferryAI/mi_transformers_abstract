...
The script is called with the following arguments:
    ...
    --type: type of transformer (vanilla or factor), note that vanilla is depreciated
...
from module_train_probe_lea import TrainTransformer
#----------------------------------------------------
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type = str, default = 'vanilla', help = "type of transformer") # vanilla or factor
    ...
    for _ in range(args.batch):
        ...
        #....................................................
        run_name = f'run_{date.today().strftime("%Y%m%d")}_{np.random.randint(100,1000)}_speaker'
        ...
        params_dataset = {
            ...
            'board_types_proba': np.array([0.95, 0.05, 0.]), # proba of generating each board type (single, double, composition)
        ...
        params_training = {
            ...
            'transformer_type': transformer_type, # should be 'factor' or 'vanilla'
            ...
            'n_embd_pos':16, # positional embedding dimensions
            ...
            'masking_patch_epoch':args.epoch, # epoch at which masking reach full proba & patch masking starts
            'masking_patch_p':0., # probability of patch masking for subsequent steps

            # new 06.23
            'board_curriculum': True, # whether to use curriculum learning
            'board_proba_epoch_start': 1000, # epoch at which board curriculum starts
            'board_proba_epoch_end': 2000, # epoch at which board curriculum ends
            'board_proba_start': [0.95, 0.05, 0.], # proba of generating each board type (single, double, composition) at start
            'board_proba_end': [0.6, 0.4, 0.], # proba of generating each board type (single, double, composition) at end
            ...
            'test_nb_instances':3, # depreciated
            'scan_patch_dim':2, # depreciated
            'scan_nb_instances':4, # depreciated
            
            'epoch2print': 25, # nb epoch between performance print
            'epoch2test': args.epoch, # depreciated
            'bool_wandb': False, # whether to use weights and biases or not
            'epoch2scan': args.epoch, # depreciated

            'save_model': args.save, # whether to save model or not
            'epoch2save': 500, # nb epoch between model save
            'run_name':run_name, # name of the run

            #....................................................
            # speaker specific
            #....................................................
            'vocab_speech_size': 128,
            'block_speech_size': 16,
            'bool_blank_speech': False, # False is better

            # vector quantization
            'vq_type': 'EMA', # vanilla, EMA

            # speaker specific
            # loss for reconstruction from speech
            'loss_rfs_epoch_start':250,
            'loss_rfs_epoch_end':1000,
            'loss_rfs_val_start':0.,
            'loss_rfs_val_end':1.,

            # loss for speech sparsity
            'loss_speech_epoch_start': 1000,
            'loss_speech_epoch_end': 3000,
            'loss_speech_val_start':0.,
            'loss_speech_val_end':0.,
        }
